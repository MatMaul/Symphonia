...
- `symphonia-codec-opus/src/celt/bands.rs`: band helper utilities (hysteresis,
  bitexact cos/log2tan, LCG).

### Not yet implemented (next targets)
- CELT VQ (`opus-1.6.1/celt/vq.c`) and band quantization (`quant_bands.c`).
- `quant_all_bands` flow from `bands.c` (decoder side first).
- Synthesis (`celt_synthesis` from `celt_decoder.c`) and CELT decoder wiring.
